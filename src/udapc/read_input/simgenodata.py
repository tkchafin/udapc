...
class SimGenotypeData(GenotypeData):
	...
	def accuracy_by_site(self, imputed):
		masked_sites = np.sum(self.mask)
		ret=np.zeros(shape=imputed.imputed.genotypes012_array.shape[1])
		for i in range(imputed.imputed.genotypes012_array.shape[1]):
			masked_sites = np.sum(self.mask[:,i])

			num_correct = np.sum(
				self.genotype_data.genotypes012_array[:,i][self.mask[:,i]]
				== imputed.imputed.genotypes012_array[:,i][self.mask[:,i]]
			)
			if masked_sites == 0:
				ret[i] = -9
			else:
				ret[i] = num_correct/masked_sites
		return(ret)

	def missingness_by_site(self, mask=False):
		ret=np.zeros(shape=self.genotype_data.genotypes012_array.shape[1])
		total=self.genotype_data.genotypes012_array.shape[0]
		# if mask == True, then only report missingness from original dataset
		if mask:
			for i in range(self.genotype_data.genotypes012_array.shape[1]):
				ret[i]=np.sum(self.mask[:,i])/total

		# if mask == False. report missingness including original and inserted missing sites
		else:
			for i in range(self.genotype_data.genotypes012_array.shape[1]):
				ret[i]=np.count_nonzero(self.genotype_data.genotypes012_array[:,i][np.invert(self.mask[:,i])] == -9)/total
		return(ret)
