...
def reverse_iupac_full(nucleotides: List[str]) -> str:
	"""Convert a list of nucleotides to the corresponding IUPAC ambiguity code.

	Args:
		nucleotides (List[str]): List of nucleotides to be converted.

	Returns:
		str: The corresponding IUPAC ambiguity code.
	"""
	...
	nucleotide_set = frozenset(nucleotide.upper() for nucleotide in nucleotides)
	return iupac_reverse.get(nucleotide_set, "N")
