...
def un_rtlda(X, c, Ninit=10, gamma=1e-6, tol=1e-6, max_iter=100, Ntry=10, center=True):
	...
	obj_old = 0  # Initialize the old objective value
	obj_new = np.finfo(float).eps # Initialize the new objective value with machine epsilon
	...
	while not np.isclose(obj_old, obj_new, atol=tol) and it < max_iter or it==0:
		...
		for j in range(Ntry+1):
			#KMeans(n_clusters=c).fit(T.T).transform(T.T)
	...
	if it == max_iter:
		print(f"Warning: The unsupervised R-RatioTraceLDA did not converge within {max_iter} iterations!")
...
plot_embeddings(T, G, W, data, labels, n_clusters)
